...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/registrations", requireAuth, async (req, res) => {
    try {
      ...
      // Send immediate confirmation email for pending registrations
      if (registrationStatus === "pending" && formation && targetUser.email) {
        void (async () => {
          try {
            ...
            console.error("Failed to send registration confirmation email", confirmError);
          }
        })();
      }

      // Send invitation email with .ics for validated registrations
